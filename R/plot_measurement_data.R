...
#'
#' This function generates various types of plots for a single measurement variable
#' (e.g., NDVI, CCI) over time or as a distribution. It allows for filtering by
#' measurement location codes and date ranges.
#'
...
plot_measurement_data <- function(output_path, variable, code = NULL, tstart = NULL, tend = NULL,
 ...
 if (plot_type == "time_series") {
  grouped_df <- if (!is.null(code)) {
   filtered_df %>%
    group_by(Date) %>%
    summarise(Avg_VAR = mean(get(variable), na.rm = TRUE), .groups = "drop")
  } else {
   filtered_df %>%
    group_by(Code, Date) %>%
    summarise(Avg_VAR = mean(get(variable), na.rm = TRUE), .groups = "drop")
  ...
  p <- ggplot(grouped_df, aes(x = Date, y = Avg_VAR, group = code, color = code)) +
   ...
   theme_minimal() +
   labs(
    title = ifelse(!is.null(code), paste0(variable, " Over Time for Code: ", code), paste0(variable, " Over Time by Code")),
    x = "Date", y = variable
   )
