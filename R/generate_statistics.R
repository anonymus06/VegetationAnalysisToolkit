...
#'
#' @importFrom dplyr group_by summarise mutate filter %>% across n
...
generate_statistics <- function(output_path, start_date = NULL, end_date = NULL, variable = "NDVI") {
  ...
  output <- capture.output({
    ...
    cat(sprintf(
     "%-10s %-10s %-10s %-10s %-10s %-5s\n",
     "Code",
     paste0("Mean_", variable),
     paste0("SD_", variable),
    ...
    for (i in 1:nrow(stats_by_group)) {
      cat(sprintf(
       "%-10s %-10.3f %-10.3f %-10.3f %-10.3f %-5d\n",
       stats_by_group$Code[i],
       stats_by_group$Mean[i],
       stats_by_group$SD[i],
       stats_by_group$Min[i],
       stats_by_group$Max[i],
       stats_by_group$N[i]))
    }
  })
  detailed_output <- capture.output({
    ...
    for (i in 1:nrow(stats_by_group_date)) {
      cat(sprintf(
       "%-10s %-10s %-10.3f %-10.3f %-10.3f %-10.3f %-5d\n",
       stats_by_group_date$Date[i],
       stats_by_group_date$Code[i],
       stats_by_group_date$Mean[i],
       stats_by_group_date$SD[i],
       stats_by_group_date$Min[i],
       stats_by_group_date$Max[i],
       stats_by_group_date$N[i]))
    }
  })
  ...
  message(paste0(
    "The statistical analysis has been completed and the results have been saved to '",
