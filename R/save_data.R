...
save_data <- function(df, output, device_type, lower_limit, upper_limit, split_code, env, variable) {
  if (device_type == "PPNP") {
    combined_df_all <- create_combined_data(df, variable,
      apply_data_filter = FALSE,
      lower_limit = NULL, upper_limit = NULL, split_code = split_code
    )
    combined_df_filtered <- create_combined_data(df, variable,
      apply_data_filter = TRUE,
  ...
  add_message(env, paste0(
    "In filtered data, values outside of bounds: ", lower_limit, " and ", upper_limit,
    " [", variable, "] were discarded!"
  ), "info")
}
